...
function VoiceWidget({ 
  supabaseUrl = 'https://yqvqoxhhxlpwkhklbuvp.supabase.co',
  supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlxdnFveGhoeGxwd2toa2xidXZwIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzA3MTIxNTcsImV4cCI6MjA0NjI4ODE1N30.5Zb7AWiVNQr6fzqIoY_AQHSsVU89aR1xhYaGJ1XVEXM'
}: VoiceWidgetProps) {
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      ...
      const { data: chatData, error: chatError } = await supabaseRef.current.functions.invoke('chat', {
        body: {
      ...
      const aiResponseText = chatData?.data?.message || chatData?.message;
      if (!aiResponseText) {
      ...
      // Convert to speech using Supabase edge function
      const { data: ttsData, error: ttsError } = await supabaseRef.current.functions.invoke('tts', {
        body: { text: aiResponseText }
      });
      ...
      const contentType = ttsData?.data?.contentType || ttsData?.contentType || 'audio/mpeg';
